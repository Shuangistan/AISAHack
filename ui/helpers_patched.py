...
def generate_mesh(  # noqa: C901
    ...
    if prune_vertices:
        ...
        ncells = numpy.concatenate([c.data for c in mesh.cells if c.data.shape[1] == 3])
        uvertices, uidx = numpy.unique(ncells, return_inverse=True)

        k = 0
        cells = []
        tri_types = {"triangle", "triangle6"}
        for cellblock in mesh.cells:
            if cellblock.type not in tri_types:
                continue
            key = cellblock.type
            n = len(cellblock.data)
            ncols = cellblock.data.shape[1]
            cells.append(
                meshio.CellBlock(key, uidx[k : k + n * ncols].reshape(cellblock.data.shape))
            )
            k += n * ncols
        mesh.cells = cells
